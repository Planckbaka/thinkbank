#!/usr/bin/env bash
# Run Python AI Redis worker on host, consuming async processing tasks.
...
export REDIS_QUEUE_NAME="${REDIS_QUEUE_NAME:-thinkbank:tasks}"
...
export LLM_MODEL="${LLM_MODEL:-Qwen/Qwen2.5-7B-Instruct-GPTQ-Int4}"

cd python-ai

if [[ -x "${ROOT_DIR}/.venv-ai/bin/python" ]]; then
  exec "${ROOT_DIR}/.venv-ai/bin/python" -m workers.asset_processor
fi

exec python3 -m workers.asset_processor
